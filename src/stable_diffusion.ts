...
export class StableDiffusionClient {
  ...
  constructor(options: StableDiffusionClientConfig) {
    ...
    if (!options.Port) fatal("Stable Diffusion Port should not be empty");
    if (!options.Path) fatal("Stable Diffusion Path should not be empty");
    this._ = options;
    this.init();
  }
  ...
  private init(): void {
    if (!this._.MaxSteps || !Number.isInteger(this._.MaxSteps)) {
      logger.warn(
        "Stable Diffuion max steps should be a integer. It has been set to default(50)"
      );
      this._.MaxSteps = 50;
    }
    if (!this._.MinSteps || !Number.isInteger(this._.MinSteps)) {
      logger.warn(
        "Stable Diffuion min steps should be a integer. It has been set to default(1)"
      );
      this._.MinSteps = 1;
    }
    if (!this._.DefaultSteps || !Number.isInteger(this._.DefaultSteps)) {
      logger.warn(
        "Stable Diffuion default steps should be a integer. It has been set to default(20)"
      );
      this._.DefaultSteps = 20;
    }
    if (!this._.DefaultCFGScale || !Number.isInteger(this._.DefaultCFGScale)) {
      logger.warn(
        "Stable Diffuion default cfg sale should be a integer. It has been set to default(7)"
      );
      this._.DefaultCFGScale = 7;
    }
    if (!this._.DefaultSampler) {
      logger.warn(
        "Stable Diffuion default sampler should not be empty. It has been set to default(Euler A)"
      );
      this._.DefaultSampler = "Euler A";
    }
    if (!this._.DefaultBatchSize) {
      logger.warn(
        "Stable Diffuion default batch size should be a integer. It has been set to default(1)"
      );
      this._.DefaultBatchSize = 1;
    }
    if (!this._.DefaultNIter) {
      logger.warn(
        "Stable Diffuion default n_iter should be a integer. It has been set to default(1)"
      );
      this._.DefaultNIter = 1;
    }
    if (!this._.DefaultSeed) {
      logger.warn(
        "Stable Diffuion default seed should be a integer. It has been set to default(-1)"
      );
      this._.DefaultSeed = 1;
    }
    if (!this._.DefaultWidth) {
      logger.warn(
        "Stable Diffuion default width should be a integer. It has been set to default(512)"
      );
      this._.DefaultWidth = 512;
    }
    if (!this._.DefualtHeight) {
      logger.warn(
        "Stable Diffuion default height should be a integer. It has been set to default(512)"
      );
  ...
  public async load(): Promise<void> {
    ...
    const { data: models } = (await this.helper.get(MODELS_URL).catch(() => {
      fatal(
        `Couldn't get models from ${this.helper.defaults.baseURL}${MODELS_URL}`
      );
    })) as any;
    ...
    // Samplers
    const { data: samplers } = (await this.helper
      .get(SAMPLERS_URL)
      .catch(() =>
        logger.fatal(
    ...
    if (
      ...
      logger.warn(
        `Couldn't find sampler that equals to default sampler ${this._.DefaultSampler}. Default sampler has been set to ${this.samplers[0].name}`
      );
    ...
    const { data: sysinfo } = (await this.helper.get(SYSINFO_URL).catch(() => {
      fatal(
        `Couldn't get sysinfo from ${this.helper.defaults.baseURL}${SYSINFO_URL}`
      );
    })) as any;
  ...
  public changeModel = async (model: Model): Promise<Model> =>
    this.helper
      .post(OPTIONS_URL, { sd_model_checkpoint: model.title })
      .then(() => (this.currentModel = model));

  public requestTxt2img = async ({
    prompt,
    negative_prompt,
    sampler_index,
    cfg_scale,
    ...
    seed,
    steps,
  }: Txt2imgParameter): Promise<string[]> => {
  ...
  public requestImg2img = async ({
    ...
    steps,
    init_images,
  }: Img2imgParameter): Promise<string[]> => {
...
const stableDiffusion = new StableDiffusionClient({
  ...
  Samplers: process.env.SD_SAMPLERS ? process.env.SD_SAMPLERS.split(",") : [],
