...
const ModelMenu: Menu = {
  ...
  build: (locale: LocaleData, builderName?: string) => {
    return new StringSelectMenuBuilder({
      custom_id: builderName
        ? `!${builderName}_${ModelMenu.name}`
        : ModelMenu.name,
      placeholder:
        stableDiffusion.models[Object.keys(stableDiffusion.models)[0]].title,
  ...
  onInteraction: async (interaction: StringSelectMenuInteraction) => {
    logger.info(
      `you select ${interaction.values} for ${interaction.guild} ${interaction.guildId}`
    );
