...
const RequestSetting1Modal: IRequestSetting1Modal = {
  ...
  build: (locale: LocaleData, parameter: Parameter) => {
    ...
    const stepsText = new TextInputBuilder({
      custom_id: "steps",
      label: `${locale.steps}${f(locale.range, {
        a: stableDiffusion._.MinSteps,
        b: stableDiffusion._.MaxSteps,
      })}`,
  ...
  onInteraction: async (interaction: ModalSubmitInteraction) => {
    ...
    if (
      checkIntegerRange(
        ...
        locale,
        locale.steps,
        stableDiffusion._.MinSteps,
        stableDiffusion._.MaxSteps,
        steps
      )
    )
      return;
