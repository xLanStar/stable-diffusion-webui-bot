...
export class BotClient extends Client {
    ...
    private init(): void {
        if (!this._.token)
            fatal("Bot client token should not be empty");
    ...
    private async loadComponents(): Promise<void> {
        for (const file of walkDir("./src/components", file => file.endsWith('.ts'))) {
            logger.info(`Load Component: ${file}`)
            const component: Component<any> = (await import(`./components/${file}`)).default;
    ...
    private async loadEmbeds(): Promise<void> {
        for (const file of walkDir("./src/embeds", file => file.endsWith('.ts'))) {
    ...
    private async loadBuilders(): Promise<void> {
        for (const file of walkDir("./src/builders", file => file.endsWith('.ts'))) {
    ...
    private async loadCommands(): Promise<void> {
        for (const file of walkDir("./src/commands", file => file.endsWith('.ts'))) {
    ...
    private async loadEvents(): Promise<void> {
        for (const file of walkDir("./src/events", file => file.endsWith('.ts'))) {
            logger.info(`Load Event: ${file}`)
            const { once, name, execute } = (await import(`./events/${file}`)).default;
